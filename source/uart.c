...
typedef volatile struct __attribute__((packed)) {
    ...
    uint32_t EVENTS_CTS;        // 0x100 CTS is activated (set 
    uint32_t EVENTS_NCTS;       // 0x104 CTS is deactivated (se
    ...
    uint32_t SHORTS;            // 0x200 Shortcuts between loca
    ...
    uint32_t BAUDRATE;          // 0x524 Baud rate. Accuracy de
    uint32_t reserved11[17];    // 0x528 - 0x568 reserved
    uint32_t CONFIG;            // 0x56C Configuration of parit
}UART_Registers;
